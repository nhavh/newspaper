...
namespace Moxiecode.TinyMCE.Web {
	...
	public class TextArea : WebControl, IPostBackDataHandler {
		...
		protected override void Render(HtmlTextWriter outWriter) {
            ...
            outWriter.Write("uploadFile :'{0}', \n",this.Config.UploadFile);            
            outWriter.Write(string.Format("theme_advanced_toolbar_align:'{0}',\n",this.Config.theme_advanced_toolbar_align));
            outWriter.Write(string.Format("theme_advanced_resizing:{0},\n", this.Config.theme_advanced_resizing));

            int count = 1;            
            outWriter.Write(string.Format("theme_advanced_buttons{1}:'{0}',\n", this.Config.theme_advanced_buttons1, count++));            
            outWriter.Write(string.Format("theme_advanced_buttons{1}:'{0}',\n", this.Config.theme_advanced_buttons2, count++));            
            outWriter.Write(string.Format("theme_advanced_buttons{1}:'{0}',\n", this.Config.theme_advanced_buttons3, count++));            
            outWriter.Write(string.Format("theme_advanced_buttons{1}:'{0}',\n", this.Config.theme_advanced_buttons4, count));

            outWriter.Write(string.Format("theme_advanced_path_location:'{0}',\n",this.Config.theme_advanced_path_location));
            outWriter.Write(string.Format("theme_advanced_toolbar_location:'{0}',\n", this.Config.theme_advanced_toolbar_location));
            outWriter.Write("onchange_callback: function(editor) { tinyMCE.triggerSave(); }");
		...
		string ScriptURI {
			get {
                ...
                outURI = (outURI.Contains("~/") == true) ? outURI : "~" + outURI;
				...
				if (this.gzipEnabled) {
					...
					foreach (TextArea area in this.FindAllTinyMCEAreas(this.Page, new ArrayList())) {
                        ...
                        area.Settings["theme"] = this.Config.theme;
                        area.Settings["plugins"] = this.Config.plugins;
