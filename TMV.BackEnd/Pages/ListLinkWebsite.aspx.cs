...
{
    public partial class ListLinkWebsite : AdminPageBase
    {
        ...
        void GetGridView_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName.ToLower() == "add")
            {
                ...
                try
                {
                    var ctrl = new LinkWebsiteController();
                    var info = new LinkWebsiteInfo();
                    foreach (System.Reflection.PropertyInfo property in CBO.GetPropertyInfo(typeof(LinkWebsiteInfo)))
                    {
