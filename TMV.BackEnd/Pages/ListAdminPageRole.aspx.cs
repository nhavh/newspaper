...
{
    public partial class ListAdminPageRole : AdminPageBase
    {
        ...
        void GetGridView_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName.ToLower() == "add")
            {
                ...
                try
                {
                    AdminPageRoleController ctrl = new AdminPageRoleController();
                    AdminPageRoleInfo info = new AdminPageRoleInfo();
                    foreach (System.Reflection.PropertyInfo property in CBO.GetPropertyInfo(typeof(AdminPageRoleInfo)))
                    {
