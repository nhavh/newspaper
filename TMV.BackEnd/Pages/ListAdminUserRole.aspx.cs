...
{
    public partial class ListAdminUserRole : AdminPageBase
    {
        ...
        void GetGridView_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName.ToLower() == "add")
            {
                ...
                try
                {
                    AdminUserRoleController ctrl = new AdminUserRoleController();
                    AdminUserRoleInfo info = new AdminUserRoleInfo();
                    foreach (System.Reflection.PropertyInfo property in CBO.GetPropertyInfo(typeof(AdminUserRoleInfo)))
                    {
